...
def analyse_image(image):
    ...
    red_pixels = cropped_pic[:, :, 0] > 135
    green_pixels = cropped_pic[:, :, 1] > 135
    blue_pixels = cropped_pic[:, :, 2] > 135
    
    mask_ocean_red = np.logical_and(cropped_pic[:, :, 0]> 5, cropped_pic[:, :, 0] <85)
    mask_ocean_green = np.logical_and(cropped_pic[:, :, 1]> 20, cropped_pic[:, :, 1] <85)
    mask_ocean_blue = np.logical_and(cropped_pic[:, :, 2]> 20, cropped_pic[:, :, 2] <85)
    
    mask_land_red = np.logical_and(cropped_pic[:, :, 0]>85, cropped_pic[:, :, 0] <135)
    mask_land_green = np.logical_and(cropped_pic[:, :, 1]> 85, cropped_pic[:, :, 1] <135)
    mask_land_blue = np.logical_and(cropped_pic[:, :, 2]> 85, cropped_pic[:, :, 2] <135)
...
for image in all_images:
    ...
    filename = write_file(image,medium_albedo)
